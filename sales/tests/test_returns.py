...
class TestReturnItemAPI:
    ...
    def test_delete_return_item(self, api_client_authenticated, test_data):
        ...
        # La eliminación de items de devolución AHORA debe afectar el stock
        initial_stock = test_data['inventory_stock'].quantity
        # Primero se crea un movimiento de entrada
        test_data['inventory_stock'].refresh_from_db()
        assert test_data['inventory_stock'].quantity == initial_stock + 1
        ...
        # Al eliminar, el stock debe volver a su estado original
        test_data['inventory_stock'].refresh_from_db()
        assert test_data['inventory_stock'].quantity == initial_stock
